...
{
	status_t status;
	FILE *inventory_file, *deletion_file, *output_file;
	...
	if ((status = open_file(&inventory_file, &deletion_file, &output_file, argv[CMD_ARG_INVENTORY_FILE], 
	...
	if ((status = eliminate(inventory_file, deletion_file, output_file, DELIMITER)) != OK)
	{
	...
	if ((status = close_files(inventory_file, deletion_file, output_file)) != OK)
	{
		print_error(status);
		return status;
	}
